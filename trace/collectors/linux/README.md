# Linux telemetry collector

Collects: auditd, journald and JSON application logs.
Evidence `source_type`: `LINUX_JSON`.
...
curl -H "Authorization: Bearer $TRACE_TOKEN" \
     ...
     -F "source=$(hostname)" \
     -F "source_type=LINUX_JSON" \
     -F "collector=linux-collector/1.0" \
